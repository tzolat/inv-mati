  ...
  try {
    ...
    for (const product of products) {
      ...
      await Notification.create({
        type: "price_change",
        message: `Price increased by ${percentage}% for ${product.name}`,
    ...
    return NextResponse.json({
      success: true,
      message: `Increased prices for ${products.length} products by ${percentage}%`,
    })
  } catch (error) {
    console.error("Error increasing prices:", error)
    return NextResponse.json({ error: "Failed to increase prices" }, { status: 500 })
  }
}
